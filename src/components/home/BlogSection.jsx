export default function BlogSection() {
    ...
    return (
        <section className="bg-slate-50 py-16 px-20" id="blog">
            <div className="max-w-[1200px] mx-auto">
                <div className="text-center mb-8">
                    <p className="text-sm font-normal text-slate-700 m-0 mb-1">Roaming Tales</p>
                    <h2 className="text-[26px] font-bold text-slate-900 m-0">Latest Travel Blog</h2>
                </div>

                <div className="grid grid-cols-4 gap-6">
                    {blogs.map((item) => (
                        <article key={item.id} className="relative rounded-2xl overflow-hidden h-64 group">
                            <div
                            ...
                            <div className="absolute inset-0 flex flex-col justify-end p-5 bg-gradient-to-t from-black/80 to-transparent">
                                ...
                                <h3 className="m-0 mb-2 text-base font-semibold text-white">{item.title}</h3>
                                {item.id === 1 && (
                ...
                <div className="flex justify-between items-center mt-8">
                    ...
                    <div className="flex gap-2">
                        <button className="w-8 h-8 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center hover:bg-slate-300 cursor-pointer">←</button>
                        <button className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center hover:bg-primary-dark cursor-pointer">→</button>
                    </div>
                </div>
            </div>
        </section>
