...
const openAiItinerary = (pinRequest, setView, request) => {
  if (!pinRequest(request)) return;
  setView("generate");
};
...
const SupplierRequests = ({
  ...
  // The request the itinerary builder is working on. Pinned on entry so that a
  // background list refresh (or the request moving to another tab) can never swap the
  // builder onto a different booking and blank the itinerary on screen.
  const [pinnedRequest, setPinnedRequest] = useState(null);

  const pinRequest = useCallback((request) => {
    const id = request?.id || request?._id;
    if (!id) return false;
    setPinnedRequest(request);
    setItineraryRequestId(id);
    setSelectedId(id);
    return true;
  }, []);
  ...
  useEffect(() => {
    ...
    setSelectedId(initialItineraryRequestId);
    setView(initialView || "generate");
    onInitialNavigationConsumed?.();
  }, [initialItineraryRequestId, initialView, onInitialNavigationConsumed]);

  // Deep links (notification / queued AI generation) only carry an id. Pin the request
  // as soon as it shows up in the list so later refreshes cannot swap the builder away.
  useEffect(() => {
    if (!itineraryRequestId) return;
    if (pinnedRequest && String(pinnedRequest.id || pinnedRequest._id) === String(itineraryRequestId)) return;
    const match = requests.find((r) => String(r.id || r._id) === String(itineraryRequestId));
    if (match) setPinnedRequest(match);
  }, [itineraryRequestId, pinnedRequest, requests]);
  ...
  // Accept / Reject / "Build Itinerary with AI" only apply to requests that have not
  // been accepted and sent yet, i.e. the New Requests tab. In Progress and Upcoming
  ...
  useEffect(() => {
    // Only re-pick a selection while the list is on screen; doing it underneath the
    // itinerary builder used to pull it onto a different booking.
    if (view !== "list") return;
  ...
  }, [subTab, filteredRequestsByTab, view]);
  ...
  // Auto-refresh every 45s — only while the list is on screen. Refreshing underneath
  // the itinerary builder is what used to make a generated itinerary vanish.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Resume carries the booking snapshot plus the id of the draft itinerary to reopen.
    const snap = resumeDraft?.payload?.requestSnapshot || resumeDraft;
    const snapId = snap?._id || snap?.id;
    if (!snapId) return;

    const resumedItineraryId =
      resumeDraft?.itineraryId ||
      snap?.itineraryId ||
      snap?.itinerary?._id ||
      (typeof snap?.itinerary === "string" ? snap.itinerary : null) ||
      null;

    // Pin it so the builder opens on this exact booking and stays there.
    pinRequest({ ...snap, itineraryId: resumedItineraryId || snap?.itineraryId });
    setResumeItineraryDraft(resumeDraft);

    setView("generate");
    onDraftConsumed?.();
  }, [resumeDraft, onDraftConsumed, pinRequest]);
  ...
  // The request driving the itinerary/builder views. When a request is pinned we keep
  // it, merging in fresher server fields for the SAME booking only — never swapping to
  // a different booking because the list reloaded or the request changed tab.
  const itineraryRequest = useMemo(() => {
    const targetId = String(itineraryRequestId || "");
    if (!targetId) return selected || null;

    const fresh = requests.find((r) => String(r.id || r._id) === targetId) || null;
    const pinnedMatches = pinnedRequest && String(pinnedRequest.id || pinnedRequest._id) === targetId;

    if (pinnedMatches) return fresh ? { ...pinnedRequest, ...fresh } : pinnedRequest;
    return fresh || selected || null;
  }, [itineraryRequestId, pinnedRequest, requests, selected]);
  ...
  useEffect(() => {
    ...
    async function loadOverviewItinerary() {
      if (!itineraryRequestKey || (view !== "itinerary" && view !== "generate")) {
      ...
      try {
        ...
        if (itineraryRecordId) {
          try {
            res = await api.get(`/itineraries/${itineraryRecordId}`);
        ...
        try {
          res = await api.get(`/itineraries/booking/${itineraryRequestKey}`);
          if (active) setOverviewItinerary(res.data);
          return;
        } catch (e) {
          // not found yet
        ...
        // Only the "Proceed" page creates the record. In the builder view the builder
        // itself owns creation — creating from both places raced and could produce two
        // itineraries for one booking.
    ...
    // Small delay so a just-completed save is reflected here.
    const timer = setTimeout(() => loadOverviewItinerary(), 600);
    return () => { active = false; clearTimeout(timer); };
  // Keyed on ids (not the request object) so background list refreshes don't
  // re-trigger a load/create cycle underneath the builder.
  }, [itineraryRequestKey, itineraryRecordId, view]);



  if (view === "itinerary" && itineraryRequest) {
    const alreadyGenerated = Array.isArray(overviewItinerary?.days) && overviewItinerary.days.length > 0;
      ...
      <div className={`space-y-6 transition-colors duration-300 ${darkMode ? "dark" : ""}`}>
        ...
        <div className={`rounded-3xl border transition-colors duration-300 px-6 py-5 shadow-sm ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-gray-100"}`}>
          ...
          <div className="flex flex-col items-center justify-center mt-6 gap-2">
            {/* "Build Itinerary with AI" is only offered while nothing has been generated
                yet. Once days exist, this opens the existing itinerary for review instead
                of regenerating over the supplier's work. */}
            <button
              type="button"
              onClick={() => {
                setGenerateMode("ai");
                setForceGenerateOnMount(!alreadyGenerated);
                setView("generate");
              }}
            ...
            >
              ...
              <span>{alreadyGenerated ? "Continue Editing Itinerary" : PROCEED_WITH_AI_LABEL}</span>
            </button>
            {alreadyGenerated && (
              <p className={`text-[11px] ${darkMode ? "text-slate-500" : "text-gray-500"}`}>
                An itinerary has already been generated for this request.
              </p>
            )}
          </div>
        </div>
              ...
              <div key={userKey} className="relative">
                ...
                >
                  ...
                  <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between border-t transition-colors pt-4" style={{ borderColor: darkMode ? "#1e293b" : "#f1f5f9" }}>
                    <div className="flex flex-col sm:flex-row flex-wrap gap-2 w-full lg:w-auto">
                      {hasAdjustment(parentRequest) ? (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            openAiItinerary(pinRequest, setView, parentRequest);
                          }}
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            openCreateItinerary(pinRequest, setView, parentRequest);
                          }}
                    ...
                    {/* Parent Button Area - Accept/Reject (New Requests only) */}
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between border-t transition-colors pt-4" style={{ borderColor: darkMode ? "#1e293b" : "#f1f5f9" }}>
                              ...
                              {/* Button Area - Accept/Reject (New Requests only) */}
                              <div className="flex items-center justify-end gap-2 w-full lg:w-auto">
                                {/* Accept Button - Shows Accepted badge when confirmed */}
                                {isNewRequestsTab && isConfirmed && (
                                  <span className="flex-1 lg:flex-none inline-flex items-center justify-center gap-1.5 rounded-full bg-emerald-100 border border-emerald-300 px-4 py-2 text-xs font-bold text-emerald-700">
                                    ✓ Accepted
                                  </span>
                                )}
                                {isNewRequestsTab && !isConfirmed && !isCancelled && (
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                ...
                                {/* Reject Button - visible unless cancelled */}
                                {isNewRequestsTab && !isCancelled && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
