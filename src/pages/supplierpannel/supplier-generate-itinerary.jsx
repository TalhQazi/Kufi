...
const SupplierGenerateItinerary = ({ darkMode, request, draft, onGoToBookings, onBack }) => {
  ...
  // Calculate total cost from actual daysData activities (not requestItemMeta)
  const totalActivitiesCost = useMemo(() => {
    const allActs = daysData.flatMap((d) => (Array.isArray(d.activities) ? d.activities : []));
    // Only sum non-auto-generated activities (user-selected ones)
    const userSelectedActs = allActs.filter((a) => !a?.isAutoGenerated);
    return userSelectedActs.reduce((sum, act) => sum + parseMoney(act?.cost), 0);
  }, [daysData]);
    ...
    setDaysData((prev) => {
      ...
      return list.map((d, dayIdx) => {
        ...
        const nextActivities = Array.from({ length: desiredCount }, (_, idx) => {
          ...
          // Set cost from backend activity price
          if (!String(patch.cost || '').trim() && meta.price > 0) patch.cost = String(meta.price);
          // Set image from backend activity
