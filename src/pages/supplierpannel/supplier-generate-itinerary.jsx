...
const SupplierGenerateItinerary = ({ darkMode, request, draft, onGoToBookings, onBack }) => {
  ...
  useEffect(() => {
    ...
    const total = parseMoney(travelDetails.budget);
    ...
    setDaysData((prev) => {
      ...
      const allActs = list.flatMap((d) => (Array.isArray(d.activities) ? d.activities : []));
      const targetActs = allActs.filter((a) => !a?.isAutoGenerated);
      const count = targetActs.length;
      if (count <= 0) return list;

      const per = total / count;
      const perStr = Number.isFinite(per) ? per.toFixed(2) : '';
      if (!perStr) return list;

      let changed = false;

      const next = list.map((d) => {
        const acts = Array.isArray(d.activities) ? d.activities : [];
        if (acts.length === 0) return d;

        const nextActs = acts.map((a) => {
          if (a?.isAutoGenerated) return a;
          if (String(a?.cost || '').trim()) return a;
          changed = true;
          return { ...a, cost: perStr };
        });
